{if is_object($v)}
    {if $v instanceof ModelCollection}
        ...
        <span onclick="$('#{$ix}').toggle();" class="collection" style="float: left; padding-right: 10px;">{$l}</span>
...
{elseif preg_match('/(^last_login$|_at$|_date$)/', $f) && is_integer($v)}
...
{else}
    ...
    {elseif is_array($v)}
        ...
        {if count($v) == 1 && isset($v[0])}
            <dl id="{$ix}" style="clear: both; margin-bottom: 0px; margin-top: 0px;">
                <dt><span style="color: red;">[0][value]</span> =&gt;</dt>
                <dd style="padding-left: 1em;">{include file="drupal_value.tpl" v=$v[0].value}</dd>
            </dl>
        {else}
            <span onclick="$('#{$ix}').toggle();" class="collection" style="float: left; padding-right: 10px;">array ({$v|@count} members)</span>
            <dl id="{$ix}" style="display: none; clear: both; margin-bottom: 0px;">
            {foreach from=$v key=n item=x}
                <dt><span style="color: red;">.{$n}</span> =&gt;</dt>
                <dd style="padding-left: 1em;">{include file="drupal_value.tpl" v=$x}</dd>
            {/foreach}
            </dl>
        {/if}
    {else}
